<template>
  <a-layout>
    <a-layout-content :style="{ background: '#fff', padding: '24px', margin: 0, minHeight: '280px' }">
      <h3 v-if="level1.length === 0">Couldn't find the relevant documentation</h3>
      <a-row>
        <a-col :span="6">
          <a-tree
        ...
        <a-col :span="18">
          <div>
            ...
            <div>
              <span>Reads:{{selectedDoc.viewCount}}</span> &nbsp; &nbsp;
              <span>Likes:{{selectedDoc.voteCount}}</span>
            </div>
          ...
          <div class="vote-div">
            <a-button type="primary" shape="round" :size="'large'" @click="voteDoc">
              <template #icon><LikeOutlined /> &nbsp;Like：{{selectedDoc.voteCount}} </template>
            </a-button>
          </div>
        </a-col>
      </a-row>
      ...
      axios.get("/doc/query/" + route.query.ebookId).then((response) => {
        ...
        if (respData.code === 0) {
          ...
          if (Tool.isNotEmpty(level1)) {
            ...
            handleQueryDocContent(level1.value[0].id);  // 并对该默认选中的文档进行一次内容query并展示出来
